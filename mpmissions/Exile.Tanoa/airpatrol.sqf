...
_spawnCenter = [7866.108,7541.369,0] ; //Center of your map -- this is Altis
_min = 50; // minimum distance from the center position (Number) in meters
_max = 6500; // maximum distance from the center position (Number) in meters
...
_airCraft = [_startPos, EAST, ["B_Plane_CAS_01_F"],[],[],[],[],[],180] call BIS_fnc_spawnGroup;
...
if (intercept) then
{
	...
	_interceptorAircraft flyInHeight 700;
...
{
    diag_log "AIRCRAFT crashed in water -- Terminating rescue sequence";
